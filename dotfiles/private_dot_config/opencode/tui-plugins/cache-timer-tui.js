...
function remainingCacheSecondsFor(api, sessionID) {
  let messages;
  try {
...
function buildSeedPrompt(api, sourceSessionID) {
  ...
  outer: for (let i = messages.length - 1; i >= 0; i--) {
    ...
    for (let j = parts.length - 1; j >= 0; j--) {
      ...
      const candidate = (typeof input?.filePath === "string" ? input.filePath : undefined) ?? (typeof input?.path === "string" ? input.path : undefined);
  ...
  const filesBlock = recentReadFiles.length > 0 ? recentReadFiles.map(p => `- ${p}`).join("\n") : "(none recorded)";
  return ["We are continuing from a previous work session. But we don't know yet what the user wants to do next.", "", `The last user request was:`, lastUserText || "(no text content recorded)", "", `The agent responded to that request with:`, lastAssistantText || "(no text content recorded)", "", `The most-recently-referenced documents were:`, filesBlock, "", "Ask the user what they would like to work on next given this context."].join("\n");
}
...
function formatWarmTime(totalSeconds) {
...
function warmLabel(exact, remainingSec, ttlSec, banked) {
...
const warningToastFiredSessions = new Set();
const coldToastFiredSessions = new Set();
...
function addPendingInteraction(sessionID, requestID) {
...
function clearPendingInteraction(sessionID, requestID) {
  if (!sessionID) return;
...
function hasPendingInteraction(sessionID) {
  const set = pendingInteractions.get(sessionID);
