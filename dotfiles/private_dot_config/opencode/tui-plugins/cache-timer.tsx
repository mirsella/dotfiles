...
function isHiddenFor(messages: ReadonlyArray<any> | undefined): boolean {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    const provider = (messages[i] as any)?.model?.providerID ?? (messages[i] as any)?.providerID;
...
function currentContextTokens(messages: ReadonlyArray<any> | undefined): number {
  if (!messages) return 0;
  for (let i = messages.length - 1; i >= 0; i--) {
    const m = messages[i];
    if (m.role !== "assistant") continue;
    const t = (m as any)?.tokens?.input;
...
function maxStackFor(contextTokens: number): number {
        ...
        const bankRefresh = () => {
          ...
          if (stacked >= cap) {
            api.ui.toast({
              ...
              title: "Stack full",
              message: cap > 0
                ? `Banked refreshes already cover the break-even window; more would cost more than a cold resume.`
                : "Context too small to price; banking paused so warming can't cost more than a cold resume.",
              duration: 4000,
            });
            return
          }
