"""Converge Beszel hub state: SMTP/sender settings, admin account, system.

Idempotent: safe to run on every nixos-rebuild switch. Reads secrets from
files given as argv (superuser password, Resend API key). Exits non-zero
with a hint when the hub is unreachable or has no superuser yet.
"""
...
def request(method, path, token=None, body=None, params=None):
    ...
    try:
        ...
        raise ApiError(f"{method} {path}: HTTP {e.code}: {e.read().decode()[:200]}")
...
def ensure_record(token, collection, filt, body, describe):
    found = request(
        "GET",
        f"/api/collections/{collection}/records",
        token=token,
        params={"filter": filt, "fields": "id"},
    )
    if found["totalItems"]:
