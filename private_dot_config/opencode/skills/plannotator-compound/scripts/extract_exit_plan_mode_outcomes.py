...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=("Which records to write to JSON/CSV outputs. Default: %(default)s"),
    )
    parser.add_argument(
...
def build_json_chunks(
    ...
    for record in records:
        ...
        if (
            current_chunk
            and current_tokens + record_tokens > max_output_tokens_per_file
        ):
...
def print_summary(
    ...
    print(f"Native denials without reason: {summary['denied_native_no_reason']}")
    print(f"Freeform native reasons: {summary['denied_native_with_freeform_reason']}")
    print(
        f"Structured native reasons: {summary['denied_native_with_structured_reason']}"
    )
    print(
        f"Non-native denials with payload: {summary['denied_non_native_with_payload']}"
    )
    print(
        f"Non-native denials without payload: {summary['denied_non_native_no_payload']}"
    )
    print(f"Captured denial reasons total: {summary['captured_denial_reasons_total']}")
    print(f"Captured freeform reasons: {summary['captured_freeform_reasons']}")
    print(f"Captured structured reasons: {summary['captured_structured_reasons']}")
    ...
    print(
        f"Human reasons from non-native denials: {summary['human_reasons_non_native']}"
    )
    print(f"Non-native / non-denial outcomes: {summary['non_native_other']}")
